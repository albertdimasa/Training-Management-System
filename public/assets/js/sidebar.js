...
function togglePinnedName() {
  if (!pinTitle) return; // Guard clause if element doesn't exist
...
pinIcon.forEach((item, index) => {
  var h6Element = item.parentNode.querySelector("h6");
  if (!h6Element) return; // Skip if h6 doesn't exist
  var linkName = h6Element.innerHTML;
